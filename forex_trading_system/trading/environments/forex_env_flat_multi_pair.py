...
pair_data = {
    'EUR_USD': pd.read_parquet('EUR_USD_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'GBP_USD': pd.read_parquet('GBP_USD_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'USD_JPY': pd.read_parquet('USD_JPY_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'USD_CHF': pd.read_parquet('USD_CHF_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'USD_CAD': pd.read_parquet('USD_CAD_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'AUD_USD': pd.read_parquet('AUD_USD_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'NZD_USD': pd.read_parquet('NZD_USD_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),


    'EUR_GBP': pd.read_parquet('EUR_GBP_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'EUR_CHF': pd.read_parquet('EUR_CHF_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'EUR_JPY': pd.read_parquet('EUR_JPY_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'EUR_CAD': pd.read_parquet('EUR_CAD_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'GBP_CHF': pd.read_parquet('GBP_CHF_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'GBP_JPY': pd.read_parquet('GBP_JPY_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'CHF_JPY': pd.read_parquet('CHF_JPY_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'AUD_JPY': pd.read_parquet('AUD_JPY_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'NZD_JPY': pd.read_parquet('NZD_JPY_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),


    'XAU_USD': pd.read_parquet('XAU_USD_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
    'XAG_USD': pd.read_parquet('XAG_USD_5min_1D_not_norm_unbiased_dll_indics_09deco_eurusd.parquet'),
}
...
class ForexTradingEnv(gym.Env):
    def __init__(
        ...
        trading_costs: Optional[TradingPairCosts1] = None,
...
class MultipairForexTradingEnv(ForexTradingEnv):
    def __init__(
        self,
        initial_balance: float = 1_000_000.0,
        trading_notional: Optional[TradingPairNotional] = None,
        ...
        # Store the pair data dictionary and available pairs
        self.all_pair_data = pair_data  # Using global pair_data
        self.pairs = list(self.all_pair_data.keys())

        # Select initial pair (will be randomized in reset)
        self.current_pair = self.pairs[0]
        initial_df = self.all_pair_data[self.current_pair]
        ...
        super().__init__(
            ...
            trading_history_size=trading_history_size,
    ...
    def reset(
        ...
        """Reset environment and switch to random currency pair."""
        # Select random pair
        self.current_pair = np.random.choice(self.pairs)

        # Update environment data for the new pair
        self.df = self.all_pair_data[self.current_pair]
        self.pair = self.current_pair

        # Update pair-specific parameters
        self.base_currency, self.quote_currency = self.current_pair.split('_')
        self.trade_size = self.trading_notional.pair_notional.get(
            self.current_pair, 100_000.0)
        self.transaction_cost = self.trading_costs.get_cost(self.current_pair)

        # Preprocess data for new pair (this updates feature_data and market_data)
        self._preprocess_data(self.df)

        # Reset parent environment
        obs, info = super().reset(seed=seed, options=options)

        # Add current pair to info
        info['current_pair'] = self.current_pair

        return obs, info


class BaseForexTradingEnv(ForexTradingEnv, ABC):
    """Abstract base class for forex trading environments."""

    @abstractmethod
    def _process_action(self, action) -> None:
        """Convert raw action to trading decision."""
        pass
...
class DiscreteForexEnv(BaseForexTradingEnv):
    """Environment with discrete action space (No Position, Long, Short)."""

    def _setup_action_space(self) -> None:
        self.action_space = spaces.Discrete(3)  # No Position, Long, Short

    def _process_action(self, action: int) -> None:
        # Convert discrete action to trading decision
        action_map = {
            0: Actions.NO_POSITION,
            1: Actions.LONG,
            2: Actions.SHORT
        }
...
class ContinuousForexEnv(BaseForexTradingEnv):
    """Environment with continuous action space for off-policy algorithms."""
    ...
    def _process_action(self, action: np.ndarray) -> None:
        ...
        position_size = float(action[0])  # Extract scalar value

        # Define thresholds for position taking
        THRESHOLD = 0.1  # Minimum absolute value to take position

        if abs(position_size) < THRESHOLD:
            return Actions.NO_POSITION
...
class ContinuousForexEnvWithSize(BaseForexTradingEnv):
    """Environment with continuous action space including position sizing."""

    def _setup_action_space(self) -> None:
        # Action space: [position_type, position_size]
        # position_type: [-1, 1] for short/long bias
        # position_size: [0, 1] for fraction of maximum position
        self.action_space = spaces.Box(
            low=np.array([-1, 0]),
            high=np.array([1, 1]),
            dtype=np.float32
    ...
    def _process_action(self, action: np.ndarray) -> Tuple[Actions, float]:
        position_type = action[0]
        position_size = action[1]

        # Scale position size to actual trade size
        scaled_size = position_size * self.max_position_size * self.trade_size

        if abs(position_type) < 0.1:
            return Actions.NO_POSITION, 0.0
        elif position_type > 0:
            return Actions.LONG, scaled_size
        else:
            return Actions.SHORT, scaled_size
